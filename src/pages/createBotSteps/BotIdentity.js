...
const BotIdentity = ({ onNext }) => {
  ...
  const goNext = (values) => {
    ...
    localStorage.setItem("botIdentity", JSON.stringify(formdata));
  };
  ...
  const getBotVoiceType = async () => {
    ...
    axios
      ...
      .then((r) => {
        //console.log(r.data.splice(8), "voice data");
  ...
  useEffect(() => {
    ...
    setInitialData(JSON.parse(localStorage.getItem("botIdentity")));
  }, []);
