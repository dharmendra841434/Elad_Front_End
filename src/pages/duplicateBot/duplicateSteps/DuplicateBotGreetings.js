...
const DuplicateBotGreetings = ({ onNext, goPrevious }) => {
  ...
  const goNext = () => {
    ...
    onNext();
    localStorage.setItem("dupbotgreeting", JSON.stringify(form));
  };

  useEffect(() => {
    setInitialData(JSON.parse(localStorage.getItem("dupbotgreeting")));
  }, []);
...
export default DuplicateBotGreetings;
