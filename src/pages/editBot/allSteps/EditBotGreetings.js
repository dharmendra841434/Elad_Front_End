...
const EditBotGreetings = ({ onNext, goPrevious }) => {
  ...
  const goNext = () => {
    ...
    onNext();
    localStorage.setItem("editbotgreeting", JSON.stringify(form));
  };

  useEffect(() => {
    setInitialData(JSON.parse(localStorage.getItem("editbotgreeting")));
  }, []);
...
export default EditBotGreetings;
