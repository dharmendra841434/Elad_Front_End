...
const EditBotSummary = () => {
  ...
  const [botIdentity, setBotIdentity] = useState(
    JSON.parse(localStorage.getItem("editBotIdentity"))
  );
  const [botGreeting, setBotGreeting] = useState(
    JSON.parse(localStorage.getItem("editbotgreeting"))
  );
  const [yourBusinessData, setYourBusinessData] = useState(
    JSON.parse(localStorage.getItem("edityourBusiness"))
  );
  const [businessFaqs, setBusinessFaqs] = useState(
    JSON.parse(localStorage.getItem("editfaqs"))
  );
  ...
  console.log(botData?.details.bot_phone_no_Data, "this is payload");
  const storeBotDetails = async () => {
    const payload = {
      ...
      details: {
        ...
        bot_phone_no_Data: botData?.details.bot_phone_no_Data,
      ...
      //change bot setting here
      // bot_settings: {
      //   getMessage: false,
      //   transferCall: false,
      //   emailsForSendingMessages: [],
      // },
      status: "active",
    };
    ...
    setLoader(true);
    await axios
      .post(`https://dev.paka-ai.com/update-bot-details`, payload)
      .then(async (result) => {
        await axios
          .get(`https://dev.paka-ai.com/bot-data/${userDetails.id}`)
          .then(({ data }) => {
      ...
      <div className=" hidden lg:block w-11/12 xl:w-4/6  mt-12 ">
        ...
        <div className=" flex my-9  mt-40">
          ...
          <button
            ...
            className=" transition-all duration-300 ease-in-out  bg-primary py-2 px-8 rounded-md mx-3 text-white"
          >
            {loader ? (
              <ThreeDots
                height="20"
      ...
      <div className=" lg:hidden pt-6">
        ...
        <div className=" flex my-9  mt-40">
          ...
          <button
            onClick={() => {
              storeBotDetails();
              // alert("edit functionality not done");
            }}
            className=" transition-all duration-300 ease-in-out  bg-primary py-2 px-8 rounded-md mx-3 text-white"
          >
            {loader ? (
